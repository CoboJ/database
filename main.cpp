...
typedef struct
{
    ...
    uint32_t page_num;
    uint32_t cell_num;
    bool end_of_table; // Indicates a position one last past the last element
} Cursor;
...
uint32_t* internal_node_child(void* node, uint32_t child_num)
{
    ...
    {
        printf("Tried to access child_num %d > num_key %d\n", child_num, num_keys);
        exit(EXIT_FAILURE);
    }
...
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key)
{
    ...
    uint32_t min_index = 0;
    uint32_t max_index = num_keys; /* there is no more child than key */
...
/*
Until we start recycling free pages will always
...
void create_new_root(Table* table, uint32_t right_child_page_num)
{
    ...
    // Root node is a new internal node with one key
